...
namespace bobweb
{
    internal static class BobErrors
    {
        /// <summary>
        /// Writes an application error to the configured error log and returns a stable user-facing code message.
        /// </summary>
        public static string LogError(AppSettings settings, Exception excp, string message, params object[] args)
        {
            string errorCode = Convert.ToString(DateTime.UtcNow.Ticks, 36).ToUpperInvariant();
            try
            {
            ...
            return string.Format("Unknown error occurred code: [{0}].", errorCode);
        }
